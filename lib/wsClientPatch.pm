...
sub _post {
    ...
    @$headers,
    # PATCH!!! the token is not always set, and the constructor of the client is somehow not
    # always called, but the token in SDK modules is always set in our environment, so just
    # set it directly!! annoying hack, but gets things to work for now.
    # ($self->{token} ? (Authorization => $self->{token}) : ()),
    #
    (Authorization => $ENV{KB_AUTH_TOKEN})
    # END PATCH.
    );
}
