package FBAFileUtil::FBAFileUtilServer;
...
sub call_method {
    ...
    my $ctx = FBAFileUtil::FBAFileUtilServerContext->new($self->{loggers}->{userlog},
...
{
    # Service FBAFileUtil requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for FBAFileUtil but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = FBAFileUtil::FBAFileUtilServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service FBAFileUtil.");
    }
...
package FBAFileUtil::FBAFileUtilServerContext;

use strict;

=head1 NAME

FBAFileUtil::FBAFileUtilServerContext
...
package FBAFileUtil::FBAFileUtilServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = FBAFileUtil::FBAFileUtilServer->new(
